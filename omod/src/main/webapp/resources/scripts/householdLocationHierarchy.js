...
// the default handler that is called when a change event occurs on any of the householdLocation field selection lists
function handleHouseholdLocationFieldChange(changedField, fieldToUpdate) {
	...
	// grab a reference to the overall householdLocation associated with the fields 
	// (we can't directly reference the householdLocation because there may be multiple householdLocationes on a single page)
	var householdLocation = fieldToUpdate.closest('.householdLocation');

	// handle the main case of updating the possible householdLocationes
	// we need to iterate through all the householdLocation hierarchy levels from top to bottom...
	...
	$j.each(householdLocationLevels, function (i, level) {		
		...
		if (i <= changedFieldIndex ) {
			searchString = searchString + householdLocation.find('.' + level).val() + "|";
		}
		// empty the other entries
		else {
			var selectList = householdLocation.find('.' + level);		
			selectList.empty();
		...
		if (i >= changedFieldIndex) {
			switchToSelectList(householdLocation.find('.' + level));
		}
	});
...
function updateOptions(fieldToUpdate, searchString, value) {	
	...
	$j.ajax({
		  type: 'POST',
		  url: pageContext + '/module/household/getChildHouseholdLocationEntries.form',
		  dataType: 'json',
...
// called from the handleHouseholdLocationFieldChange to handle the case when a user selects the "blank" option from the list of options
function handleSelectBlank(field) {
	var fieldLevelIndex = findIndexOfLevel(field.attr('class'));
	var householdLocation = field.closest('.householdLocation');
	
	// iterate through all the householdLocation hierarchy levels
	$j.each(householdLocationLevels, function (i, level) {		
		// we want to skip all the hierarchy levels before the level of the householdLocation field we are operating on
...
// called from handleHouseholdLocationFieldChange to handle the case when a user selects "Other" from the list of options
...
// find the numerical index of a level within the householdLocation hierarchy
function findIndexOfLevel(level) {
	for(var i=0; i<householdLocationLevels.length; i++){ 
	       if(householdLocationLevels[i] == level) {
	            return i;
	       }
	}
}
